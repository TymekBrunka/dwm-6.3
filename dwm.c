...
enum { SchemeNorm, SchemeSel, SchemeBarSel }; /* color schemes */
...
static void togglefullscr(const Arg *arg); /*patch*/
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		if (m->sel) {
			...
			drw_setscheme(drw, scheme[m == selmon ? SchemeBarSel : SchemeNorm]); /*custom scheme for bar selection*/
